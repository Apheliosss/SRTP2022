    ...
    def __init__(self, insize, outsize):
        ...
        while math.pow(2, int(math.log2(self.lastsize))) >= max(128, outsize + 1):
            if i == 1:
                ...
                self.thissize = int(
                    math.pow(2, int(math.log2(self.lastsize)) - 1))

            self.fc.add_module(
                "full"+str(i), nn.Linear(self.lastsize, self.thissize))
        ...
        self.fc.add_module(
            "full"+str(i), nn.Linear(self.lastsize, self.outsize))
...
def process_df(my_set, varName):
    ...
    dataset['lon'][dataset['lon'] <
                   0] = dataset['lon'][dataset['lon'] < 0].copy() + 360.0
    cor_df['xcor'] = dataset['lon']
    cor_df['ycor'] = dataset['lat']

    sample_pt = np.array([[110.0, 0.0], [290.0, 0.0], [
                         110.0, 70.0], [290.0, 70.0]])
...
def train(epoch):
    ...
    for data, coef, label in train_loader:
        ...
        data, label = data.view(
            data.shape[0], -1), label.view(data.shape[0], -1)
...
def val(epoch):
    ...
    with torch.no_grad():
        ...
        print('\r Epoch: {} \tValidation Loss: {:.6f} \tR2: {:.6f}'.format(
            epoch, val_loss, best_loss))
...
def full_img(path: str):
    ...
    for i in range(1, varNum, 1):
        dataset.loc[:, varName[i]] = (
            dataset[varName[i]].copy() - mean_li[i] + 1.0) / std_li[i]

    img_data = MYDataset(process_df(
        dataset, varName=varName), varNum=varNum, gpu=is_gpu)
...
# for i in range(0, varNum, 1):
#     train_set.loc[:, varName[i]] = (
#         train_set[varName[i]].copy() - mean_li[i] + 1.0) / std_li[i]
#     test_set.loc[:, varName[i]] = (
#         test_set[varName[i]].copy() - mean_li[i] + 1.0) / std_li[i]


train_data = MYDataset(process_df(
    my_set=train_set, varName=varName), varNum, gpu=is_gpu)
test_data = MYDataset(process_df(
    my_set=test_set, varName=varName), varNum, gpu=is_gpu)
...
mean_li[0] = data['fCO2'].mean()
std_li[0] = data['fCO2'].std()
